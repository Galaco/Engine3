...
void SGraphics::Update()
{
	...
	this->getCurrentWindow()->swapBuffers();
}

bool SGraphics::Initialise()
{
	this->initialize();
	return true;
}

void SGraphics::FixedUpdate(float dt)
{
	
}

void SGraphics::initialize(){
	this->m_pWindow = new Window(Config::_WINDOWWIDTH, Config::_WINDOWHEIGHT, Config::_FULLSCREEN, "3D Sokoban");
...
Window* SGraphics::getCurrentWindow() {
	return m_pWindow;
}